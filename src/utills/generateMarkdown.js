export function generateMarkdown({
  ...
  if (problemUnderstanding?.trim()) {
    md += `## ${sectionNumber++}. Understand the Problem\n\n`;
    problemUnderstanding.split("\n").forEach((line) => {
  ...
  // Input & Output
  if (inputOutput?.input?.trim() || inputOutput?.output?.trim()) {
    md += `## ${sectionNumber++}. Input & Output\n\n`;
  ...
  if (examples && examples.length) {
    ...
    if (filteredExamples.length) {
      md += `## ${sectionNumber++}. Examples & Edge Cases\n\n`;
